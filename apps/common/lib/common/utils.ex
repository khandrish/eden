defmodule Exmud.Common.Utils do
  ...
  def normalize_ecto_errors(errors),
    do: Enum.map(errors, fn {key, {error, _}} -> {key, error} end)

  def normalize_multi_result({:ok, _results}), do: :ok

  def normalize_multi_result({:ok, results}, desired_result),
    do: {:ok, results[desired_result] || desired_result}
  ...
  def normalize_repo_result({:error, changeset}, _),
    do: {:error, normalize_ecto_errors(changeset.errors)}

  def normalize_insert_result({:ok, record}), do: {:ok, record.id}

  def normalize_insert_result({:error, changeset}),
    do: {:error, normalize_ecto_errors(changeset.errors)}
  ...
  @compression_threshold_bytes Application.get_env(:common, :byte_size_to_compress)
