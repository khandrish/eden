defmodule Exmud.Game.Contributions.Command.Middleware.MatchCommand do
  ...
  If more than one Command is matched, a MultiMatch Command will be used instead. This Command will be responsible for
  setting up the appropriate Command Set on the Object to allow for a choice to be made between the matching Commands.
  """

  # TODO: Sort out the logic in this middleware

  alias Exmud.Engine.Command.ExecutionContext
  import Exmud.Engine.Constants
  import Exmud.Engine.Utils, only: [engine_cfg: 1]
  ...
  def execute(execution_context) do
    ...
    case match_commands(input, commands) do
      ...
      multiple_commands ->
        execution_context =
          ExecutionContext.put(execution_context, command_multi_match_key(), multiple_commands)
