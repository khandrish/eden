defmodule Exmud.Engine.Command.Middleware do
  ...
  In the same way that the entire command processing pipeline can be replaced by changing the config, a per-command
  override can be provided at the time of execution. Overriding the processing pipeline in this way will only impact the
  single command being processed while the rest will continue to use the default pipeline provided in the config.
  """

  @doc """
  The middleware callback function operates on an '%Exmud.Engine.Command.Execution{}' struct, returning an updated
