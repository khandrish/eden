defmodule Exmud.GameObjectTest do
  ...
  describe "game object tests: " do
    ...
    test "tag tests", %{oid: oid} = _context do
      ...
      assert MapSet.member?(MapSet.new(GameObject.list(tags: "foobar")), oid) == false
      assert GameObject.add_tag(oid, "bar") == :ok
      assert GameObject.has_tag?(oid, "bar") == true
      assert MapSet.member?(MapSet.new(GameObject.list(tags: ["foo", "bar"])), oid) == true
      assert GameObject.remove_tag(oid, "foo") == :ok
      assert GameObject.has_tag?(oid, "foo") == false
    end

    test "attribute tests", %{oid: oid} = _context do
      assert GameObject.has_attribute?(oid, "foo") == {:ok, false}
      assert GameObject.get_attribute(oid, "foo") == {:error, :no_such_attribute}
      assert GameObject.add_attribute(oid, "foo", "bar") == :ok
      assert GameObject.get_attribute(oid, "foo") == {:ok, "bar"}
      assert GameObject.has_attribute?(oid, "foo") == {:ok, true}
    ...
    test "alias tests", %{oid: oid} = _context do
      ...
      assert MapSet.member?(MapSet.new(GameObject.list(aliases: ["foobar"])), oid) == false
      assert GameObject.add_alias(oid, "bar") == :ok
      assert GameObject.has_alias?(oid, "bar") == true
      assert MapSet.member?(MapSet.new(GameObject.list(aliases: ["foo", "bar"])), oid) == true
