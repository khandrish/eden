defmodule ExmudWeb.SimulationCallbackControllerTest do
  ...
  describe "create simulation_callback" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.simulation_callback_path(conn, :create), simulation_callback: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.simulation_callback_path(conn, :create), simulation_callback: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Simulation callback"
    end
  end
  ...
  describe "edit simulation_callback" do
    ...
    test "renders form for editing chosen simulation_callback", %{conn: conn, simulation_callback: simulation_callback} do
  ...
  describe "update simulation_callback" do
    ...
    test "redirects when data is valid", %{conn: conn, simulation_callback: simulation_callback} do
      conn = put(conn, Routes.simulation_callback_path(conn, :update, simulation_callback), simulation_callback: @update_attrs)
      assert redirected_to(conn) == Routes.simulation_callback_path(conn, :show, simulation_callback)

      conn = get(conn, Routes.simulation_callback_path(conn, :show, simulation_callback))
      assert html_response(conn, 200)
    end

    test "renders errors when data is invalid", %{conn: conn, simulation_callback: simulation_callback} do
      conn = put(conn, Routes.simulation_callback_path(conn, :update, simulation_callback), simulation_callback: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Simulation callback"
    end
  end
  ...
  describe "delete simulation_callback" do
    ...
    test "deletes chosen simulation_callback", %{conn: conn, simulation_callback: simulation_callback} do
